...
async def test_list_nichos(client):
    ...
    response = await client.get("/api/templates/nichos")
...
async def test_list_templates_by_nicho(client):
    ...
    response = await client.get("/api/templates?nicho=sdr")
    assert response.status_code == 200

@pytest.mark.asyncio
async def test_get_default_template_not_found(client):
    """Testa busca de template padrão inexistente"""
    response = await client.get("/api/templates/nicho/inexistente/default")
