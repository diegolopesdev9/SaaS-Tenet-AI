...
export default function Agencias() {
  ...
  const generatePassword = () => {
    ...
    setForm({ ...form, admin_senha: password });
  };
  ...
  const handleDelete = async (id, nome) => {
    if (!confirm(`Tem certeza que deseja deletar a agência "${nome}"? Isso também removerá todos os usuários vinculados.`)) return;
        ...
        <div className="mb-6 p-6 bg-amber-50 border border-amber-200 rounded-lg">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-amber-600">Guarde essas informações - a senha não será exibida novamente!</p>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <div className="flex items-center gap-2">
                <code className="flex-1 px-3 py-2 bg-white border border-amber-200 rounded text-sm">{createdUser.nome}</code>
              </div>
            </div>
            <div>
          ...
          <p className="mt-4 text-xs text-amber-600">
            ⚠️ O usuário será obrigado a alterar a senha no primeiro login.
          </p>
        </div>
      )}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Email da Agência *</label>
                  <input
                    type="email"
                    value={form.email}
            ...
            <div className="flex justify-end gap-3 pt-4 border-t">
              <button
                type="button"
                onClick={() => { setShowForm(false); setCreatedUser(null); }}
